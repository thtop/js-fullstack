...
User.prototype.validate = function() {
    return new Promise(async (resolve, reject) => {
        ...
        // Only if username is valid then check to see if it's alerady taken
        if (this.data.username.length > 2 && this.data.username.length < 31 && validator.isAlphanumeric(this.data.username)) {
            let usernameExists = await usersCollection.findOne({username: this.data.username});
            if (usernameExists) {
                this.errors.push('That username is alerady taken.');
            }
        }
    
        // Only if email is valid then check to see if it's alerady taken
        if (validator.isEmail(this.data.email)) {
            let emailExists = await usersCollection.findOne({email: this.data.email});
            if (emailExists) {
                this.errors.push('That email is alerady being used.');
            }
        }
